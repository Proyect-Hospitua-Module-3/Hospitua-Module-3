...
**Por qué esta prioridad**: El check-out es el punto donde se conoce con certeza el número real de noches hospedadas y se cierra el ciclo financiero de la estancia. Sin este cierre, la liquidación "Preliminary" del check-in nunca se formaliza como factura definitiva.

**Prueba independiente**: Se puede simular la emisión del evento "Registrar Check-out" desde el Módulo 2 para una estancia con liquidación "Preliminary" de Canal Directo, y comprobar de forma independiente que Módulo 3 recalcula el hospedaje final, cierra la liquidación y emite una factura con numeración consecutiva.

**Escenarios de aceptación**:

1. **Escenario**: Cierre de liquidación cuando la salida coincide con la fecha programada
	- **Dado** que existe una liquidación "Preliminary" para una reserva de Canal Directo y el actor Módulo 2 emite "Registrar Check-out" con la fecha de salida real igual a la programada
	- **Cuando** el Módulo 3 procesa el evento y ejecuta "Generar liquidación"
	- **Entonces** confirma el hospedaje ya calculado en el check-in, calcula el IVA definitivo, transiciona la liquidación a estado "Final" y emite la factura final con numeración consecutiva oficial.
...
**Prueba independiente**: Se puede emitir un evento "Registrar Check-out" para una estancia OTA con liquidación "Preliminary", y verificar que la comisión final se recalcula sobre el hospedaje definitivo, el IVA se ajusta en consecuencia, y la liquidación cierra con el balance neto correcto.

**Escenarios de aceptación**:

1. **Escenario**: Comisión OTA recalculada sobre hospedaje definitivo sin cambios de fecha
	- **Dado** que existe una liquidación "Preliminary" de canal OTA con comisión pactada del 15% y el check-out ocurre en la fecha programada
	- **Cuando** el Módulo 3 ejecuta "Generar liquidación"
...
- **Check-out sin liquidación "Preliminary" asociada**: Si no existe una liquidación registrada para el identificador de reserva/estancia informado en el evento, el sistema debe rechazar el check-out y no crear una liquidación nueva desde cero.
...
- **Reenvío de evento de Check-out ya procesado (evento duplicado)**: Si se recibe nuevamente el evento "Registrar Check-out" para una estancia cuya liquidación ya está "Final", el sistema debe responder con la liquidación ya cerrada sin volver a ejecutar el cálculo ni modificar el total definitivo.
- **Falla en dependencias al momento del cierre (tarifa para noches adicionales o alícuota de IVA)**: Si no es posible resolver la tarifa dinámica de una noche adicional o el porcentaje de IVA vigente, el cierre debe abortarse de forma atómica y la liquidación debe permanecer en estado "Preliminary", no cerrarse con datos parciales.
...
- **FR-003**: El sistema DEBE localizar la liquidación previamente registrada en `Modulo3` para el identificador de reserva/estancia recibido; si no existe una liquidación "Preliminary" asociada, DEBE rechazar el evento.
...
- **FR-009**: El sistema DEBE transicionar el estado de la liquidación de "Preliminary" a "Final" una vez consolidados el hospedaje, la comisión (si aplica) y el IVA definitivos.
...
- **FR-013**: El sistema DEBE rechazar el procesamiento del evento "Registrar Check-out" y no cerrar la liquidación si la fecha real de salida es anterior a la fecha de check-in registrada, si no existe liquidación "Preliminary" asociada, o si no es posible resolver la tarifa dinámica o el porcentaje de IVA requeridos para el cierre.
- **FR-014**: El sistema DEBE garantizar un manejo idempotente ante reintentos del evento "Registrar Check-out" para una estancia cuya liquidación ya está "Final", respondiendo con la liquidación final ya existente sin recalcular ni duplicar el cierre.
...
- **Liquidación de estancia (cierre)**: La misma liquidación "Preliminary" del check-in, actualizada al estado "Final" con los valores definitivos de hospedaje, comisión (si aplica), IVA y total liquidado final.
...
- **BR-002**: Cierre obligatorio: cada evento válido de check-out cierra la única liquidación "Preliminary" asociada a la estancia, a través de la relación `<<include>>` hacia `Generar liquidacion`.
...
- **BR-008**: Cierre atómico: si alguna dependencia de cálculo falla al momento del cierre, la liquidación permanece en estado "Preliminary" y no se cierra con datos parciales o estimados.
...
- **SC-001**: El 100% de los eventos válidos de "Registrar Check-out" con liquidación "Preliminary" asociada consolidan un cierre con hospedaje, comisión (si aplica) e IVA definitivos.
...
- **SC-004**: El 100% de los eventos sin liquidación "Preliminary" asociada, con fechas inconsistentes, o con dependencias de cálculo no resueltas, son rechazados sin cerrar liquidaciones parciales.
