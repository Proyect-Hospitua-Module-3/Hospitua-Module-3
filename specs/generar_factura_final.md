...
**Prueba independiente**: Se puede invocar "Generar factura final" en el contexto de una liquidación recién abierta en el check-in (canal directo u OTA) y verificar que el sistema entregue un borrador con el desglose de hospedaje, comisión (si aplica) e IVA preliminar, sin número de factura oficial.
...
2. **Escenario**: Prefactura para liquidación de canal OTA en el check-in
   - **Dado** que la liquidación abierta en el check-in pertenece a un canal OTA con comisión ya descontada
...
3. **Escenario**: La prefactura se actualiza mientras la estancia sigue abierta
   - **Dado** que existe una prefactura generada en el check-in y la liquidación preliminar se recalcula por un cambio de fechas o de canal antes del check-out
...
2. **Escenario**: Emisión de factura definitiva tras cierre de liquidación de canal OTA
   - **Dado** que la liquidación cerrada corresponde a una reserva OTA con comisión definitiva ya descontada
...
- Se solicita la factura definitiva sobre una liquidación que aún no ha cerrado (sigue en estado abierto): el sistema no debe asignar numeración oficial sin un cierre previo confirmado.
- La liquidación transiciona a estado anulado antes del check-out: la prefactura asociada se descarta y nunca deriva en una factura definitiva.
...
- **FR-008**: El sistema DEBE reemplazar el contenido de la prefactura cada vez que la liquidación preliminar se recalcule antes del check-out, sin asignarle numeración oficial en ninguna de esas actualizaciones.
...
- **FR-012**: El sistema DEBE rechazar la emisión de la factura definitiva y no asignar numeración oficial si faltan los datos tributarios mínimos del cliente responsable, si la liquidación asociada no existe, o si la liquidación no se encuentra en estado cerrado.
- **FR-013**: El sistema DEBE descartar la prefactura sin generar nunca una factura definitiva cuando la liquidación asociada transicione a estado anulado.
...
- **Prefactura**: Documento en borrador generado al ejecutar "Generar factura final" en el check-in; sin numeración oficial, mutable mientras la liquidación preliminar siga abierta.
...
- **BR-005**: La prefactura refleja siempre el estado más reciente de la liquidación preliminar, y deja de existir en su forma de borrador en el momento en que se emite la factura definitiva.
...
- **BR-007**: Una liquidación anulada nunca deriva en una factura definitiva; cualquier prefactura asociada queda descartada.

## Requisitos no funcionales

- **NFR-001**: Determinismo: para la misma liquidación cerrada y los mismos datos tributarios, la factura definitiva generada debe ser siempre idéntica en desglose y total.
...
- **SC-001**: El 100% de las liquidaciones abiertas en el check-in generan una prefactura visible como borrador, sin numeración oficial.
- **SC-002**: El 100% de las liquidaciones cerradas en el check-out generan una factura definitiva con numeración consecutiva oficial única.
...
- **SC-005**: El 100% de los intentos de emitir una factura definitiva sin datos tributarios mínimos o sin liquidación cerrada son rechazados sin asignar numeración oficial.
- **SC-006**: El 100% de las facturas definitivas permanecen inmutables tras su emisión durante pruebas de auditoría posterior.
- **SC-007**: El 100% de las prefacturas asociadas a liquidaciones anuladas quedan descartadas sin derivar en una factura definitiva.
