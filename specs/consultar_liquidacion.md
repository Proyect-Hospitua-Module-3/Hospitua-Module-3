...
### Historia de usuario 2 - Módulo 2 confirma el resultado financiero tras el check-out (Prioridad: P1)

Como Módulo 2 (módulo de reservas), quiero consultar la liquidación de una estancia después de emitir el evento de check-out, para confirmar que Módulo 3 procesó el evento correctamente y poder reflejar el monto correspondiente a recepción o al huésped.

**Por qué esta prioridad**: Módulo 2 emite el evento que dispara `Generar liquidación` de forma asíncrona; sin una forma de consultar el resultado, no puede cerrar su propio flujo operativo con la certeza de que la liquidación se generó.

**Prueba independiente**: Se puede emitir el evento de check-out para una estancia y verificar que, al consultarla inmediatamente después como Módulo 2, el resultado refleje exactamente el desglose producido por ese evento.

**Escenarios de aceptación**:

1. **Escenario**: Confirmación tras un check-out
   - **Dado** que Módulo 2 acaba de emitir el evento `Registrar Check-out` para una estancia
...
2. **Escenario**: Consulta de una estancia sin check-out
   ...
   - **Cuando** Módulo 2 consulta su liquidación
...
Como actor autorizado (Módulo 2 u OTA), quiero que cada respuesta de `Consultar liquidación` indique con claridad si la liquidación ya existe (`Final`) o si la estancia todavía no ha llegado al check-out, para no tratar por error la ausencia de datos como un ingreso neto en cero.
...
- **Consulta de liquidación**: Solicitud de un actor autorizado (`Módulo 2`, `OTA`) para obtener el desglose de la liquidación `Final` de una estancia, o la confirmación explícita de que aún no existe.
