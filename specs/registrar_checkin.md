...
2. **Escenario**: Generación de liquidación inicial para estancia de Canal Directo que cruza temporadas
	- **Dado** que el actor Módulo 2 emite el evento "Registrar Check-in" para una reserva de Canal Directo cuyas fechas programadas comprenden noches de distintas temporadas (baja, regular o alta)
...
3. **Escenario**: Cálculo de comisión sobre estancia con tarifa dinámica
	- **Dado** que una reserva originada en OTA incluye noches con ajuste de temporada (baja, regular o alta)
...
- **Check-in el mismo día de inicio de una temporada**: La primera noche de la estancia debe ser clasificada según la temporada correspondiente (baja, regular o alta) por "Revisar temporada del año" y liquidada con la regla de tarifa dinámica respectiva.
...
- **FR-002**: El sistema DEBE validar que el payload del evento "Registrar Check-in" contenga los siguientes datos mínimos: identificador único de la reserva/estancia, identificador del tipo de habitación o habitación asignada, fecha de entrada (check-in), fecha programada de salida (check-out), canal de origen de la reserva (`Canal Directo` o `OTA`) y datos mínimos de identificación tributaria del cliente responsable de la facturación (nombre o razón social, y número de identificación tributaria/documento fiscal), exclusivamente para fines de emisión de factura — sin incluir nacionalidad, tipo de visa, ni ningún otro dato de control migratorio, que permanece bajo responsabilidad del Módulo 2.
...
- **FR-005**: Al ejecutar `Generar liquidacion`, el sistema DEBE inicializar una cuenta de liquidación única asociada a la estancia con estado "Preliminary" (o en curso), registrando la fecha y hora de apertura.
...
- **FR-010**: Al ejecutar `Descontar comision OTA`, el sistema DEBE invocar obligatoriamente `Consultar porcentaje de comision OTA` (`<<include>>`) para utilizar el porcentaje de comisión pactado, el cual es suministrado siempre por `Modulo2` en el payload del evento; el Módulo 3 NO mantiene una tabla propia de convenios de comisión por OTA.
...
- **FR-012**: Dentro de `Generar liquidacion`, el sistema DEBE invocar obligatoriamente el caso de uso `Generar factura final` (`<<include>>`) para estructurar una prefactura o borrador tributario preliminar asociado a la liquidación "Preliminary" (estableciendo los rubros facturables iniciales sin emitir un documento fiscal definitivo ni asignar numeración consecutiva oficial, reservándose esto para el cierre de estancia en `Registrar Check-out`).
...
- **FR-019**: El sistema DEBE permitir transicionar una liquidación en estado "Preliminary" a un estado "Cancelled" cuando el actor `Modulo2` notifique la anulación del check-in correspondiente, preservando el registro completo sin eliminarlo, sin requerir un caso de uso adicional para ello.
- **FR-020**: El sistema NO DEBE capturar, procesar ni persistir datos de control migratorio o archivos .TXT de SIRE, respetando la frontera de responsabilidad con `Modulo2`.
- **FR-021**: El sistema NO DEBE modificar estados físicos de ocupación de habitaciones ni controlar disponibilidad de inventario, respetando la frontera de responsabilidad con `Modulo1`.
...
- **Liquidación de estancia** (`Generar liquidacion`): Registro maestro financiero en Módulo 3 vinculado unívocamente a la estancia. Almacena el estado ("Preliminary" o "Cancelled"), fechas, subtotales de hospedaje, deducciones de intermediarios, impuestos y saldo liquidable, contemplando exclusivamente el hospedaje dinámico, la deducción de comisión OTA (si aplica) y el IVA, sin incluir otros consumos o cargos adicionales.
...
- **BR-005**: Fórmula de liquidación de comisión OTA: La deducción por comisión de intermediario se calcula como `- (Valor Hospedaje * % Comisión)`. El porcentaje utilizado en esta fórmula es siempre el valor suministrado por `Modulo2` en el evento "Registrar Check-in"; el Módulo 3 no resuelve ni almacena convenios de comisión por OTA de forma independiente.
...
- **BR-007**: Estado de la liquidación al Check-in: La liquidación se inicializa en estado "Preliminary", contemplando exclusivamente el hospedaje dinámico, la deducción de comisión OTA (si aplica) e IVA, y sirviendo de base para que al momento del `Registrar Check-out` se consolide el balance final y cierre de facturación.
...
- **BR-010**: Anulación de check-in: si `Modulo2` notifica la anulación inmediata del check-in después de generada la liquidación inicial, el sistema transiciona el estado de la liquidación a "Cancelled", conservando el registro completo para trazabilidad. Esta transición no requiere un caso de uso independiente, dado que en el momento del check-in la liquidación aún no incluye una factura fiscal formal emitida ni un cobro real ejecutado a la OTA.

## Requisitos no funcionales

- **NFR-001**: Determinismo y exactitud monetaria: El cálculo de la liquidación debe ser completamente determinista; para las mismas fechas, tarifa base, canal y porcentaje de comisión, el desglose e importe total deben ser idénticos, respetando la precisión de decimales y reglas de redondeo definidas.
