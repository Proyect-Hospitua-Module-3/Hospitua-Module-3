...
- **FR-005**: Al ejecutar `Generar liquidacion`, el sistema DEBE inicializar una cuenta de liquidación única asociada a la estancia con estado "Preliminar" (o en curso), registrando la fecha y hora de apertura.
...
- **FR-012**: Dentro de `Generar liquidacion`, el sistema DEBE invocar obligatoriamente el caso de uso `Generar factura final` (`<<include>>`) para estructurar una prefactura o borrador tributario preliminar asociado a la liquidación preliminar (estableciendo los rubros facturables iniciales sin emitir un documento fiscal definitivo ni asignar numeración consecutiva oficial, reservándose esto para el cierre de estancia en `Registrar Check-out`).
...
- **FR-019**: El sistema DEBE permitir transicionar una liquidación en estado "Preliminar" a un estado "Anulado" cuando el actor `Modulo2` notifique la anulación del check-in correspondiente, preservando el registro completo sin eliminarlo, sin requerir un caso de uso adicional para ello.
...
- **Liquidación de estancia** (`Generar liquidacion`): Registro maestro financiero en Módulo 3 vinculado unívocamente a la estancia. Almacena el estado ("Preliminar" o "Anulado"), fechas, subtotales de hospedaje, deducciones de intermediarios, impuestos y saldo liquidable, contemplando exclusivamente el hospedaje dinámico, la deducción de comisión OTA (si aplica) y el IVA, sin incluir otros consumos o cargos adicionales.
...
- **BR-007**: Estado de la liquidación al Check-in: La liquidación se inicializa en estado "Preliminar", contemplando exclusivamente el hospedaje dinámico, la deducción de comisión OTA (si aplica) e IVA, y sirviendo de base para que al momento del `Registrar Check-out` se consolide el balance final y cierre de facturación.
...
- **BR-010**: Anulación de check-in: si `Modulo2` notifica la anulación inmediata del check-in después de generada la liquidación inicial, el sistema transiciona el estado de la liquidación a "Anulado", conservando el registro completo para trazabilidad. Esta transición no requiere un caso de uso independiente, dado que en el momento del check-in la liquidación aún no incluye una factura fiscal formal emitida ni un cobro real ejecutado a la OTA.
