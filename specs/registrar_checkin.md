...
- **FR-019**: El sistema DEBE permitir transicionar una liquidación en estado "Abierta" a un estado "Anulada" cuando el actor `Modulo2` notifique la anulación del check-in correspondiente, preservando el registro completo sin eliminarlo, sin requerir un caso de uso adicional para ello.
...
- **Liquidación de estancia** (`Generar liquidacion`): Registro maestro financiero en Módulo 3 vinculado unívocamente a la estancia. Almacena el estado ("Abierta" o "Anulada"), fechas, subtotales de hospedaje, deducciones de intermediarios, impuestos y saldo liquidable, contemplando exclusivamente el hospedaje dinámico, la deducción de comisión OTA (si aplica) y el IVA, sin incluir otros consumos o cargos adicionales.
...
- **BR-010**: Anulación de check-in: si `Modulo2` notifica la anulación inmediata del check-in después de generada la liquidación inicial, el sistema transiciona el estado de la liquidación a "Anulada", conservando el registro completo para trazabilidad. Esta transición no requiere un caso de uso independiente, dado que en el momento del check-in la liquidación aún no incluye una factura fiscal formal emitida ni un cobro real ejecutado a la OTA.
