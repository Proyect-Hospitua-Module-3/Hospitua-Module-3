...
2. **Escenario**: La liquidación `Preliminary` permanece estable mientras la estancia sigue activa
   - **Dado** que existe una liquidación `Preliminary` generada en el check-in y la estancia aún no ha tenido check-out
   - **Cuando** transcurre tiempo o se vuelve a consultar la liquidación sin que Módulo 2 haya emitido el evento de check-out
   - **Entonces** el sistema mantiene sin cambios el hospedaje, el canal de origen y el porcentaje de comisión OTA capturados en el check-in, sin recalcular la liquidación hasta que ocurra el evento de check-out
...
- **FR-001**: El sistema DEBE generar la liquidación de una estancia únicamente cuando sea invocado (`<<include>>`) por `Registrar Check-in` (liquidación `Preliminary`) o por `Registrar Check-out` (liquidación `Final`), casos de uso que son los que reciben directamente los eventos del Módulo 2; sin que haya ocurrido alguno de estos dos eventos no debe existir liquidación.
- **FR-002**: El sistema DEBE obtener el valor de hospedaje de la estancia a partir del resultado del cálculo de tarifa dinámica para el tipo de habitación y el período correspondiente.
- **FR-003**: El sistema DEBE invocar obligatoriamente (`<<include>>`) el caso de uso `Aplicar tarifa dinámica` para obtener el valor de hospedaje de cada noche de la estancia, tanto al generar la liquidación `Preliminary` (noches programadas del check-in) como al recalcular la liquidación `Final` (noches efectivamente transcurridas, incluidas las de una extensión de estancia, conforme a FR-024).
- **FR-004**: El sistema DEBE identificar el canal de origen de la reserva (directo o intermediario OTA) antes de determinar si corresponde un descuento de comisión.
- **FR-005**: El sistema DEBE asumir canal directo cuando la reserva no tenga un canal de origen registrado.
- **FR-006**: El sistema DEBE obtener el porcentaje de comisión pactado desde los datos de la reserva suministrados por el Módulo 2 en el payload del check-in cuando el canal de origen sea un intermediario OTA, y conservarlo sin cambios para usarlo tanto en la liquidación `Preliminary` como en la `Final`; el sistema no mantiene una tabla propia de convenios de comisión por OTA ni vuelve a consultar el porcentaje en el check-out.
- **FR-007**: El sistema DEBE descontar del valor de hospedaje la comisión correspondiente únicamente cuando la reserva provenga de un intermediario OTA, ejecutando para ello el caso de uso extendido `Descontar comisión OTA` (`<<extend>> si y solo si hay intermediario`); dicho caso de uso incluye obligatoriamente (`<<include>>`) a `Consultar porcentaje de comisión OTA` únicamente en el primer cálculo de la estancia (check-in), y reutiliza el porcentaje ya fijado —sin volver a incluir esa consulta— en los recálculos posteriores del check-out, conforme a FR-006.
- **FR-008**: El sistema NO DEBE aplicar ningún descuento de comisión, ni ejecutar `Descontar comisión OTA`, cuando la reserva sea de canal directo o no tenga canal de origen registrado.
- **FR-009**: El sistema DEBE calcular el ingreso neto de la liquidación como el valor de hospedaje menos la comisión OTA aplicable, cuando corresponda.
- **FR-010**: El sistema DEBE generar una liquidación en estado `Preliminary`, visible como estimado, al registrar el check-in de la estancia.
- **FR-011**: El sistema DEBE generar una liquidación en estado `Final` al registrar el check-out de la estancia, independiente de cualquier liquidación `Preliminary` previa.
- **FR-012**: El sistema DEBE mantener sin cambios la liquidación `Preliminary` (hospedaje, canal de origen y porcentaje de comisión OTA) mientras la estancia permanezca activa, sin recalcularla hasta que Módulo 2 emita el evento de check-out; Módulo 3 no gestiona reservas ni dispone de un evento propio para modificar las fechas de una estancia en curso.
- **FR-013**: El sistema DEBE mantener una única liquidación `Final` por estancia; no debe generar una segunda liquidación `Final` para la misma estancia.
- **FR-014**: El sistema DEBE entregar un desglose de la liquidación que incluya el valor de hospedaje, el canal de origen, el porcentaje y valor de la comisión OTA (si aplica), y el ingreso neto resultante.
- **FR-015**: El sistema NO DEBE incluir el Impuesto al Valor Agregado (IVA) dentro del ingreso neto de la liquidación.
- **FR-016**: El sistema DEBE rechazar la generación de la liquidación cuando falte el valor de hospedaje o el porcentaje de comisión requerido para un canal OTA.
- **FR-017**: El sistema DEBE devolver un motivo identificable y accionable cuando rechace la generación de una liquidación.
- **FR-018**: El sistema DEBE identificar la reserva, el tipo de habitación, las fechas de la estancia y el canal de origen asociados a cada liquidación generada.
- **FR-019**: El sistema DEBE permitir consultar posteriormente una liquidación ya generada sin recalcularla, para que su valor pueda reutilizarse en la generación de la factura final.
- **FR-020**: El sistema NO DEBE modificar el estado de disponibilidad, ocupación o bloqueo de la habitación como efecto de generar una liquidación.
- **FR-021**: El sistema DEBE transicionar una liquidación `Preliminary` a estado `Cancelled` cuando el Módulo 2 notifique la anulación del check-in correspondiente antes del check-out, conservando el registro completo sin eliminarlo.
- **FR-022**: El sistema NO DEBE generar una liquidación `Final` para una estancia cuya liquidación fue transicionada a estado `Cancelled`.
- **FR-023**: El sistema DEBE invocar obligatoriamente (`<<include>>`) el caso de uso `Generar factura final` como parte de la generación de cada liquidación, produciendo una prefactura cuando la liquidación resultante sea `Preliminary` y una factura fiscal definitiva cuando sea `Final`.
- **FR-024**: El sistema DEBE calcular el valor de hospedaje de la liquidación `Final` sobre el número de noches efectivamente transcurridas entre el check-in y la fecha real de salida, el cual puede diferir del número de noches programadas utilizado para la liquidación `Preliminary`.
...
- **Comisión OTA**: Porcentaje pactado con un intermediario, suministrado por el Módulo 2 como parte de los datos de la reserva, vigente para el canal de origen de la reserva; el monto a descontar se obtiene mediante el caso de uso extendido `Descontar comisión OTA`, que incluye a `Consultar porcentaje de comisión OTA` únicamente en el primer cálculo de la estancia (check-in) y reutiliza el porcentaje ya fijado en los recálculos del check-out, sin volver a consultarlo (ver `descontar_comision_ota.md`, `consultar_porcentaje_comision_ota.md`).
- **Valor de hospedaje**: Resultado obtenido mediante el `<<include>>` obligatorio a `Aplicar tarifa dinámica` para el tipo de habitación y el período de la estancia, usado como base de la liquidación (ver `aplicar_tarifa_dinamica.md`).
...
- **BR-001**: La liquidación solo puede generarse a partir del registro de check-in (`Preliminary`) o de check-out (`Final`); no existe liquidación sin que se haya registrado alguno de estos dos eventos.
- **BR-002**: El ingreso neto de la liquidación es igual al valor de hospedaje calculado mediante la tarifa dinámica, menos la comisión OTA cuando la reserva proviene de un intermediario.
- **BR-003**: La comisión OTA solo se descuenta cuando el canal de la reserva es un intermediario, mediante la relación `<<extend>>` hacia `Descontar comisión OTA`; en canal directo, o cuando no hay canal de origen registrado, la comisión es cero y ese caso de uso extendido no se ejecuta.
- **BR-004**: El valor de hospedaje usado en la liquidación se obtiene mediante la relación `<<include>>` hacia `Aplicar tarifa dinámica`, y debe coincidir con el resultado calculado para la misma habitación y el mismo período; la liquidación no recalcula la tarifa por su cuenta.
...
- **BR-006**: La liquidación generada en el check-in tiene carácter `Preliminary`, es visible como estimado, y permanece fija mientras la estancia siga activa; solo se recalcula y consolida con carácter `Final` cuando ocurre el check-out.
- **BR-007**: Debe existir una única liquidación `Final` por estancia; el sistema no genera una segunda liquidación `Final` para la misma estancia.
- **BR-008**: El porcentaje de comisión OTA aplicado en la liquidación `Final` es el mismo capturado a partir del payload del check-in y persistido en la liquidación `Preliminary`; el sistema no vuelve a consultarlo ni a recalcularlo en el check-out, incluso si el porcentaje pactado con la OTA cambia posteriormente en Módulo 2.
- **BR-009**: Un dato obligatorio faltante o inconsistente debe detener la generación de la liquidación, sin producir un ingreso neto estimado o parcial.
- **BR-010**: La disponibilidad, ocupación o bloqueo de la habitación no forma parte de la liquidación y no se modifica al generarla.
- **BR-011**: Una liquidación `Preliminary` transiciona a estado `Cancelled` cuando se anula el check-in correspondiente antes del check-out; una liquidación `Final` nunca transiciona a `Cancelled`, dado que el check-out ya cerró la estancia.
- **BR-012**: Una liquidación `Cancelled` conserva su registro completo para trazabilidad, pero no puede recalcularse ni derivar en una liquidación `Final`.
- **BR-013**: Toda liquidación generada incluye (`<<include>>`) obligatoriamente a `Generar factura final`; no existe una liquidación `Preliminary` sin su prefactura asociada ni una liquidación `Final` sin su factura fiscal definitiva asociada.
- **BR-014**: El valor de hospedaje de la liquidación `Final` puede diferir del estimado en la liquidación `Preliminary` cuando la fecha real de salida difiera de la programada (salida anticipada o extensión de estancia); en ambos casos, el hospedaje `Final` refleja las noches efectivamente transcurridas.
- **BR-015**: El canal de origen de la reserva es el recibido de Módulo 2 en el check-in (`Canal Directo` u `OTA`) y es inmutable durante toda la estancia; el sistema no lo modifica ni lo vuelve a consultar en el check-out, y no gestiona correcciones de canal, ya que esa información es propiedad exclusiva de Módulo 2.

## Requisitos no funcionales

- **NFR-001**: La generación de la liquidación debe ser determinista: la misma reserva, con las mismas reglas y datos vigentes, debe producir siempre el mismo resultado.
...
- **NFR-004**: La exactitud monetaria de la liquidación debe ser consistente con la precisión y el redondeo definidos para el cálculo de tarifa dinámica.
...
- **NFR-007**: La solución debe mantener la integridad referencial entre la reserva, la tarifa aplicada y la comisión aplicada en cada liquidación.
...
- **SC-002**: El 100% de las liquidaciones de reservas con canal OTA reflejan el descuento de comisión con el mismo porcentaje capturado en el check-in, sin variación entre la liquidación `Preliminary` y la `Final`.
...
- **SC-007**: El 100% de las estancias activas (con check-in y sin check-out) exponen únicamente liquidaciones en estado `Preliminary`, visibles como estimado.
...
- **SC-009**: El 100% de las liquidaciones `Preliminary` cuyo check-in fue anulado transicionan a estado `Cancelled`, conservan su registro, y nunca derivan en una liquidación `Final`.
