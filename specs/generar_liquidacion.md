...
- **FR-003**: El sistema DEBE identificar el canal de origen de la reserva (directo o intermediario OTA) antes de determinar si corresponde un descuento de comisión.
- **FR-004**: El sistema DEBE asumir canal directo cuando la reserva no tenga un canal de origen registrado.
- **FR-005**: El sistema DEBE obtener el porcentaje de comisión pactado desde los datos de la reserva suministrados por el Módulo 2 en el payload del check-in cuando el canal de origen sea un intermediario OTA, y conservarlo sin cambios para usarlo tanto en la liquidación `Preliminary` como en la `Final`; el sistema no mantiene una tabla propia de convenios de comisión por OTA ni vuelve a consultar el porcentaje en el check-out.
- **FR-006**: El sistema DEBE descontar del valor de hospedaje la comisión correspondiente únicamente cuando la reserva provenga de un intermediario OTA, ejecutando para ello el caso de uso extendido `Descontar comisión OTA` (`<<extend>> si y solo si hay intermediario`); dicho caso de uso incluye obligatoriamente (`<<include>>`) a `Consultar porcentaje de comisión OTA` únicamente en el primer cálculo de la estancia (check-in), y reutiliza el porcentaje ya fijado —sin volver a incluir esa consulta— en los recálculos posteriores del check-out, conforme a FR-005.
- **FR-007**: El sistema NO DEBE aplicar ningún descuento de comisión, ni ejecutar `Descontar comisión OTA`, cuando la reserva sea de canal directo o no tenga canal de origen registrado.
...
- **FR-009**: El sistema DEBE generar una liquidación en estado `Preliminary`, visible como estimado, al registrar el check-in de la estancia.
- **FR-010**: El sistema DEBE generar una liquidación en estado `Final` al registrar el check-out de la estancia, independiente de cualquier liquidación `Preliminary` previa.
- **FR-011**: El sistema DEBE mantener sin cambios la liquidación `Preliminary` (hospedaje, canal de origen y porcentaje de comisión OTA) mientras la estancia permanezca activa, sin recalcularla hasta que Módulo 2 emita el evento de check-out; Módulo 3 no gestiona reservas ni dispone de un evento propio para modificar las fechas de una estancia en curso.
- **FR-012**: El sistema DEBE mantener una única liquidación `Final` por estancia; no debe generar una segunda liquidación `Final` para la misma estancia.
- **FR-013**: El sistema DEBE entregar un desglose de la liquidación que incluya el valor de hospedaje, el canal de origen, el porcentaje y valor de la comisión OTA (si aplica), y el ingreso neto resultante.
- **FR-014**: El sistema NO DEBE incluir el Impuesto al Valor Agregado (IVA) dentro del ingreso neto de la liquidación.
- **FR-015**: El sistema DEBE rechazar la generación de la liquidación cuando falte el valor de hospedaje o el porcentaje de comisión requerido para un canal OTA.
- **FR-016**: El sistema DEBE devolver un motivo identificable y accionable cuando rechace la generación de una liquidación.
- **FR-017**: El sistema DEBE identificar la reserva, el tipo de habitación, las fechas de la estancia y el canal de origen asociados a cada liquidación generada.
- **FR-018**: El sistema DEBE permitir consultar posteriormente una liquidación ya generada sin recalcularla, para que su valor pueda reutilizarse en la generación de la factura final.
- **FR-019**: El sistema NO DEBE modificar el estado de disponibilidad, ocupación o bloqueo de la habitación como efecto de generar una liquidación.
- **FR-020**: El sistema DEBE transicionar una liquidación `Preliminary` a estado `Cancelled` cuando el Módulo 2 notifique la anulación del check-in correspondiente antes del check-out, conservando el registro completo sin eliminarlo.
- **FR-021**: El sistema NO DEBE generar una liquidación `Final` para una estancia cuya liquidación fue transicionada a estado `Cancelled`.
- **FR-022**: El sistema DEBE invocar obligatoriamente (`<<include>>`) el caso de uso `Generar factura final` como parte de la generación de cada liquidación, produciendo una prefactura cuando la liquidación resultante sea `Preliminary` y una factura fiscal definitiva cuando sea `Final`.
- **FR-023**: El sistema DEBE calcular el valor de hospedaje de la liquidación `Final` sobre el número de noches efectivamente transcurridas entre el check-in y la fecha real de salida, el cual puede diferir del número de noches programadas utilizado para la liquidación `Preliminary`.
...
- **BR-004**: El valor de hospedaje usado en la liquidación debe coincidir con el resultado del cálculo de tarifa dinámica para la misma habitación y el mismo período; la liquidación no recalcula la tarifa por su cuenta.
